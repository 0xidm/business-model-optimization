...
class BusinessModel:
    def __init__(self, starting_deposits, starting_growth_pct, average_user_yield, starting_pol, average_protocol_yield, protocol_fee_pct, buyback_rate_pct, monthly_swap_pressure_pct, credit_utilization=0.5, periods_in_year=12, periods_in_simulation=48):

        self.starting_deposits = starting_deposits
        self.starting_growth_pct = starting_growth_pct
    ...
    def plot(self, filename="surplus.png"):
        ...
        assert len(df) == self.periods_in_simulation
    ...
    def current_growth_pct(self, month):
        ...
        return self.starting_growth_pct * _scaling_factor
    ...
    def new_deposits(self, month):
        if month == 0:
            ...
            return self.total_deposits(month-1) * self.current_growth_pct(month-1)
    ...
    def total_treasury(self, month):
        ...
        result = _treasury + self.net_new_pol(month) - self.buybacks(month) + self.surplus(month-1)
        return result
    ...
    def cost_of_lp(self, month):
        return self.current_lp_apr(month) * self.cumulative_base_asset_required(month) / self.periods_in_year
    
    @functools.cache
    def tvl(self, month):
        _tvl = self.total_treasury(month) + self.total_deposits(month) + self.total_sages(month)
        if _tvl < 0:
            return 0
        else:
            return _tvl
